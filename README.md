...
对应 X 链接：`https://x.com/<用户ID>/status/<帖子ID>`，
上例为 `https://x.com/<user-id>/status/<tweet-id>`。
...
Docker 部署建议把数据目录放在媒体根目录的 `.data` 子目录（如 `<media-root>/.data`），
