...
本项目以 **GNU General Public License v2.0（GPL-2.0）** 发布，全文见 [LICENSE](LICENSE)。

你可以自由使用、修改与分发本项目；但如果分发的是**修改后的版本**，需要同样以 GPL-2.0 开源并提供源码。
注意：这是一条"传染性"条款——把本项目的代码并入其他程序时，整个作品也需要按 GPL-2.0 分发；
反过来，把它并入更宽松的许可项目则不被允许。

Copyright (C) 2026 Xlikes contributors
