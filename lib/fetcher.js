...
async function fetchText(tweetId, user) {
  ...
  for (const fn of sources) {
    ...
    if (r && r.text) return { status: 'ok', text: r.text, source: r.source };
...
async function fetchFxtwitter(tweetId, user) {
  ...
  if (!res.ok) return null;
  const data = await res.json();
  const text = data && data.tweet && data.tweet.text;
  return text ? { text, source: 'fxtwitter' } : null;
}
...
async function fetchVxtwitter(tweetId, user) {
  ...
  return text ? { text, source: 'vxtwitter' } : null;
}
