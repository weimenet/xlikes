// JSON 文件存储：媒体索引 + 帖子文案缓存 + 设置（数据量小，零依赖）
const fs = require('fs');
...
function loadSettings(dataDir) {
  try {
    return JSON.parse(fs.readFileSync(path.join(dataDir, SETTINGS_FILE), 'utf8'));
  } catch {
    return {};
  }
}
...
function saveSettings(dataDir, settings) {
  atomicWrite(path.join(dataDir, SETTINGS_FILE), settings);
}
...
module.exports = { loadIndex, saveIndex, loadPosts, savePosts, loadSettings, saveSettings };
